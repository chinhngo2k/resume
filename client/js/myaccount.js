...
(async () => {
    const res = await fetch(`http://localhost/resume/server/api/personal/getPersonalByAccount.php?acc_id=${id}`);
    data = await res.json();
    ...
    data.forEach((element) => {
        ...
        <div>
            <a href="http://localhost/resume/client/editresume.html?personal_id=${element.personal_id}">chinh sua</a>
            <button type="button" class="btn-delete" id="${element.personal_id}">xoa</button>
        </div>
    </div>`;
...
listPersonal.onclick = async (e) => {
    ...
    if(item.getAttribute('type') == 'button'){
        ...
        await fetch(`http://localhost/resume/server/api/resume/deleteResume.php?per_id=${id}`);
        location.reload();
    }
}